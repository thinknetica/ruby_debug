...
namespace :avatars do
  ...
  task load_inline: :environment do
  ...
  task load_in_treads: :environment do
    uri = URI("http://localhost:3000/pages/avatar")
    threads_limit = 4
    threads = []
    User.find_each do |user|
  ...
  task load_in_http_pool: :environment do
  ...
  task load_with_handle_interrupt: :environment do
    uri = URI("http://localhost:3000/pages/avatar")
    http = Net::HTTP.new(uri.host, uri.port)
    post = Net::HTTP::Post.new(uri)
    threads_limit = 2
  ...
  desc 'Saves avatars for users in threads'
  task activerecord_handle_interrupt_demo: :environment do
  ...
  task binding_pry_handle_interrupt: :environment do
    th = Thread.new do
      Thread.handle_interrupt(Exception: :never) do
        begin
          a = 0
